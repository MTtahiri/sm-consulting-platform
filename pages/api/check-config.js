...
export default async function handler(req, res) {
  ...
  const config = {
    serviceAccountEmail: process.env.GOOGLE_SHEETS_CLIENT_EMAIL || "sm-consulting@sm-consulting-447413-i7.iam.gserviceaccount.com",
    sheetId: process.env.GOOGLE_SHEETS_ID || "1H4bSpOvOEMQ8ftg3aZyf8XJmtDFi9JIN8WaRTuwtUzQ",
  ...
  if (!config.privateKeyExists) {
    config.issues.push("❌ GOOGLE_SHEETS_PRIVATE_KEY manquante");
  }
